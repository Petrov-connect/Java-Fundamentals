    ...
    public static void main(String[] args) {
        ...
        while (!"Season end".equals(input=scan.nextLine())){
            if(input.contains(" -> ")){
                ...
                String player=command[0]; String position=command[1]; int points=Integer.parseInt(command[2]);
                ...
                String playerOne=fight[0]; String playerTwo=fight[1];
                if(!playerOne.equals(playerTwo)&&playerInfo.containsKey(playerOne)&&playerInfo.containsKey(playerTwo)){
                    int totalPointsPlayerOne=0; int totalPointsPlayerTwo=0;
        ...
        playerInfo.entrySet().stream().sorted((player1,player2)->
                ...
                System.out.printf("%s: %d skill%n",entry.getKey(),entry.getValue().values().stream().mapToInt(Integer::intValue).sum());
