...
public class TakeSkipRope {
    public static void main(String[] args) {
        ...
        text.forEach(character -> { if (Character.isDigit(character))
        { numbers.add(Integer.parseInt(String.valueOf(character)));} else{ nonNumbers.add(String.valueOf(character));}});
        StringBuilder output=new StringBuilder(); int count=0;
        for (int i =0; i<numbers.size();  i++) {
            if (i % 2 == 0) {
                for (int j = 0; j < numbers.get(i); j++) {
                    if(count!=nonNumbers.size()) {
                        output.append(nonNumbers.get(count)); count++;
                    }
                }
            } else { count += numbers.get(i);
            }
        }
