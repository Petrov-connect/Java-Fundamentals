...
public class SoftUniCoursePlanning {
    public static void main(String[] args)  {
        ...
        for (int i = 0; i < coursesList.size(); i++) { coursesList.set(i,coursesList.get(i).trim());}
        String input;
        while (!"course start".equals(input = scan.nextLine())) {
            String[] commands = input.split(":+");
            switch (commands[0]) {
                case "Add":
                    if (!coursesList.contains(commands[1])) { coursesList.add(commands[1]);}
                    break;
                case "Insert":
                    if (!coursesList.contains(commands[1]) && (Integer.parseInt(commands[2]) >= 0 && Integer.parseInt(commands[2]) < coursesList.size())) {
