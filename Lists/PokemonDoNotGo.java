...
public class PokemonDoNotGo {
    public static void main(String[] args) {
        ...
        List<Integer> pokemonList= Arrays.stream((scan.nextLine().split("\\s+"))).map(Integer::parseInt).collect(Collectors.toList());
        int sumRemovedElements=0;
        while (!pokemonList.isEmpty()) {
            int index = Integer.parseInt(scan.nextLine());
            int valueElement;
            if(index<0){
                valueElement=pokemonList.get(0); sumRemovedElements+=valueElement;
                ...
                valueElement=pokemonList.get(pokemonList.size()-1); sumRemovedElements+=valueElement;
                if(pokemonList.size()>1){
                ...
                valueElement=pokemonList.get(index); sumRemovedElements+=valueElement;
