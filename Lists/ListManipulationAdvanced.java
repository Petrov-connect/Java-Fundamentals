...
public class ListManipulationAdvanced {
    public static void main(String[] args) {
        ...
        while (!("end").equals(input=scan.nextLine())) {
            ...
            switch (command[0]) {
                case "Contains": checkTheContents(numbersList,command);
                break;
                case "Print": printAllEvenOddNumbers(numbersList,command);
                break;
                case "Get": printSumOfNumbers(numbersList);
                    break;
                case "Filter": printListWhitCondition(numbersList,command);
                    break;
            }
        }
    }
    private static void printListWhitCondition(List<Integer> numbersList, String[] command) {
        ...
        for (Integer integer : numbersList) {
            switch (command[1]) {
                case "<":  condition = integer < Integer.parseInt(command[2]);
                    break;
                case ">":  condition = integer > Integer.parseInt(command[2]);
                    break;
                case "<=": condition = integer <= Integer.parseInt(command[2]);
