        ...
        for (int i = 0; i <input.size()-1 ; i++) {
            if((input.get(i).equals("*")||input.get(i).equals(":"))&&(input.get(i).equals(input.get(i+1)))
                ...
                while (((input.get(index).charAt(0) < 123 && input.get(index).charAt(0) > 96))){
                    countLowerLetters++; emojiValid.append(input.get(index)); index++;
                }
                if((input.get(index).equals(input.get(i)))&&(input.get(index).equals(input.get(index+1)))&&(countLowerLetters>2)){
