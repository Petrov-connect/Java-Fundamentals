...
public class RawData2 {
    public static void main(String[] args) {
        ...
        if (command.equals("fragile")) {
            carList.stream().filter(e -> e.getCargo().getType().equals("fragile")).forEach(e -> { Tire[] tires = e.getTires();
            if (Arrays.stream(tires).anyMatch(tireSet -> tireSet.getPressure() < 1)) { System.out.println(e.getModel()); } });

        } else {
            carList.forEach(e -> { if (e.getCargo().getType().equals("flamable") && e.getEngine().getPower() > 250)
    ...
    static class Car {
        ...
        Tire[] getTires() { return new Tire[]{tire1, tire2, tire3, tire4}; }
